...
* AMOLECOIN_ADDR - The amolecoin node's address. You must include a scheme. Defaults to http://127.0.0.1:9981

CLI Options:
* -api-only - Don't serve static content from ./dist, only proxy the amolecoin node
...
const (
	...
	defaultAmolecoinAddr  = "http://127.0.0.1:9981"

	// timeout for requests to the backend amolecoin node
	amolecoinRequestTimeout = time.Second * 30
...
var (
	...
	amolecoinAddr  *url.URL // override with envvar AMOLECOIN_ADDR.  Must have scheme, e.g. http://
...
func init() {
	...
	amolecoinAddrString := os.Getenv("AMOLECOIN_ADDR")
	if amolecoinAddrString == "" {
		amolecoinAddrString = defaultAmolecoinAddr
	}

	origURL, err := url.Parse(amolecoinAddrString)
	if err != nil {
		log.Println("AMOLECOIN_ADDR must have a scheme, e.g. http://")
		log.Fatalln("Invalid AMOLECOIN_ADDR", amolecoinAddrString, err)
	}

	if origURL.Scheme == "" {
		log.Fatalln("AMOLECOIN_ADDR must have a scheme, e.g. http://")
	}

	amolecoinAddr = &url.URL{
...
func buildAmolecoinURL(path string, query url.Values) string {
	rawQuery := ""
	if query != nil {
	...
	u := &url.URL{
		Scheme:   amolecoinAddr.Scheme,
		Host:     amolecoinAddr.Host,
		Path:     path,
		RawQuery: rawQuery,
	}
...
type APIEndpoint struct {
	...
	AmolecoinPath    string   `json:"amolecoin_path"`
...
func (s APIEndpoint) ServeHTTP(w http.ResponseWriter, r *http.Request) {
	...
	amolecoinURL := buildAmolecoinURL(s.AmolecoinPath, query)

	log.Printf("Proxying request %s to amolecoin node %s with timeout %v", r.URL.String(), amolecoinURL, amolecoinRequestTimeout)

	c := &http.Client{
		Timeout: amolecoinRequestTimeout,
	}

	resp, err := c.Get(amolecoinURL)
	if err != nil {
		msg := "Request to amolecoin node failed"
		log.Println("ERROR:", msg, amolecoinURL)
	...
	if s.ExplorerPath == "/api/coinmarketcap" {
		...
		if err := json.NewDecoder(resp.Body).Decode(&cs); err != nil {
			...
			log.Println("ERROR:", msg, amolecoinURL, err)
			http.Error(w, msg, http.StatusInternalServerError)
			return
		}
	...
	if n, err := io.Copy(w, resp.Body); err != nil {
		msg := "Copying response from amolecoin node to client failed"
		if n != 0 {
...
var apiEndpoints = []APIEndpoint{
	{
		...
		AmolecoinPath:    "/api/v1/coinSupply",
...

	{
		...
		AmolecoinPath:    "/api/v1/blockchain/metadata",
...

	{
		...
		ExampleRequest: "http://explorer.amole.cc/api/blocks?start=1891&end=1892",
		ExampleResponse: `{
    "blocks": [
        {
            "header": {
...

	{
		...
		AmolecoinPath:    "/api/v1/transaction",
...

	{
		...
		AmolecoinPath:    "/api/v1/pendingTxs",
	...
	},
	{
		...
		AmolecoinPath:    "/api/v1/addresscount",
	...
	},
	{
		...
		AmolecoinPath:    "/api/v1/transactions",
...
<title>Amolecoin Explorer API Documentation</title>
<style type="text/css">
...
<h1>Amolecoin Explorer API Documentation</h1>

<div>
<p>
<p>The Amolecoin Explorer API proxies a subset of a Amolecoin node's API.</p>
<p>All endpoints start with /api</p>
<p>Further information about an endpoint can be found at the Amolecoin repo.</p>
<p>Amolecoin Github:<a href="https://github.com/amolecoin/amolecoin">https://github.com/amolecoin/amolecoin</a>.</p>
<p>Amolecoin Explorer Github: <a href="https://github.com/amolecoin/amolecoin-explorer">https://github.com/amolecoin/amolecoin-explorer</a></p>
</p>
</div>
...
{{ if .AmolecoinPath }}
<p>Internal amolecoin node path:</p>
<p class="example"><code class="inline">{{ .AmolecoinPath }}</code></p>
{{ end }}
...
func main() {
	...
	for _, e := range apiEndpoints {
		...
		log.Printf("%s proxied to %s with args %v", e.ExplorerPath, e.AmolecoinPath, e.QueryArgs)
	}
	...
	log.Printf("Running amolecoin explorer on http://%s", explorerHost)
